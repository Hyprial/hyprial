...
class AgentRegistry:
    ...
    def __init__(
        ...
        *,
        ...
        #: to log (this module deliberately has no logging seam).
    ...
    def _lifecycle_create_payload(
        self, payload: object, resource_row: sqlite3.Row | None
    ) -> dict[str, object]:
    ...
    def save(self, agent: Agent) -> Agent:
        ...
        with self._lock, self._db:
            existing = self._db.execute(
                "SELECT owner, machine, uri, hosted_by FROM agents WHERE actor = ?",
                (agent.actor,),
            ).fetchone()
    ...
    def _insert_statement(agent: Agent) -> tuple[str, tuple[Any, ...]]:
        return (
            "INSERT INTO agents (actor, owner, machine, uri, cwd, provider, "
